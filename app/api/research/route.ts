  ...
  try {
    ...
    const aiResponse = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are assisting a qualified automotive technician. Summarize only the supplied search extracts. Clearly distinguish likely diagnostic directions from confirmed facts, mention safety-critical uncertainty, tell the mechanic to verify against official service information, and cite source numbers only as plain square brackets like [1] inline in the prose, never any other citation format. Write in short paragraphs and "- " bullet points only. Never use markdown tables or pipe characters. Never invent torque values, procedures, part fitment or diagnoses.' },
